...
## Context
As financial ledgers grow into millions of journal entries and outbox events, several operational failure modes emerge:
...
4. **AI Reasoning Predictability in Regulated Systems:** Unconstrained LLM reasoning in financial triage workflows introduces non-deterministic execution risks.
...
2. **Outbox Lease Expiration & Worker Reclaiming:**
   * Outbox events in PROCESSING whose locked_at timestamp exceeds the lease window (5 minutes) are automatically reclaimed alongside PENDING events via `findClaimableEventsWithSkipLocked`.
...
4. **Decoupled IncidentReasoningEngine:**
   ...
   * DeterministicIncidentReasoningEngine serves production traffic with predictable rule evaluation.
...
* **Resilience:** Automated lease recovery from worker crashes without manual operational intervention.
